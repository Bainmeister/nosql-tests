...
public class TokuMXPessimist extends TokuMX{
	...
	public ActionRecord balanceTransfer(String key1, String key2, int waitMillis) {
		...
		try{
			...
			BasicDBObject searchQueryOne = new BasicDBObject().append("_id",keyObj1);
			searchQueryOne.append("$eq", new BasicDBObject().append("balance", oldBalance1));
			collection.update(searchQueryOne, set1);
